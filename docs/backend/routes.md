...
- **Chat (ChatController)**: `GET /chat/sessions`, `GET /chat/sessions/{session}`, `POST /chat/sessions/{session}/messages`, `POST /chat/session-for-client` (ensure/create chat session for a client and return it with messages).
...
- **TwilioController**: `GET /twilio/whatsapp-senders` (available sender numbers/Messaging Service SID). Public webhook: `POST /twilio/webhook/whatsapp` (no auth) for Twilio status callbacks + inbound WhatsApp messages.
