...
- **Tenant Scoping & Strict Admin Bank Isolation**: Restricted `canAccessAllBanks()` strictly to `SUPER_ADMIN`. An `ADMIN` role is a tenant/institution administrator and is strictly isolated to their assigned bank(s) across Clients, Campaigns, Live Chat (WABAs and chat sessions), Banks, and Users. Revoked `bypass_bank_scoping` from non-super-admin roles via migration `2026_09_12_150000_revoke_bypass_bank_scoping_from_non_super_admin_roles.php`. Removed frontend hardcoded blanket overrides for `ADMIN` in `hasPermission()`, `canAccessAllBanks`, and `canChooseBank`.
